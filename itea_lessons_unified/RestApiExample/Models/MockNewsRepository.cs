...
namespace BasicInfo.Models
{
    public class MockNewsRepository : INewsRepository 
    {
        ...
        {
            News delNew = _news.Where(x=> x.Id==id).Single();
            if(delNew!=null)
            {
                _news.Remove(delNew);
            }
        ...
        public void UpdateNews(News _new)
        {
            _news[_new.Id]=_new;
        }

        public void PatchNews(int id, JsonPatchDocument<News> _new)
        {
            var existNew = GetNews().FirstOrDefault(x => x.Id == id);
            _new.ApplyTo(existNew);
        }
    }
